...
class OpenGL2Frame(val parentPanel: GLJPanel) : GLEventListener {
    ...
    fun startJobs(coroutineScope: CoroutineScope) {
        jobs.add(coroutineScope.launch(context = Dispatchers.Main) {
            TestViewModel.subscribePointCloud(collector = { pointCloud ->
                pointCloudData = pointCloud
            })
        })

        jobs.add(coroutineScope.launch(context = Dispatchers.Main) {
            TestViewModel.subscribeLocalization(collector = { localization ->
        ...
        jobs.add(coroutineScope.launch(context = Dispatchers.Main) {
            TestViewModel.subscribeObstacle(collector = { obstacle ->
                ...
                for (obj in obstacle) {
                    val polygon = getPolygonFromObstacle(obj)
                    objects[obj.id] = polygon
        ...
        jobs.add(coroutineScope.launch(context = Dispatchers.Main) {
            TestViewModel.subscribeDummy(collector = { dummys ->
                for (info in objectsInfo) {
                    info.value.ttc = "NULL"
                    info.value.risk = "NULL"
                }
            })
        })

        jobs.add(coroutineScope.launch(context = Dispatchers.Main) {
            PointCloudViewModel.subscribePointCloudOption { option ->
                pointCloudOption = option
            }
        })

        jobs.add(coroutineScope.launch(context = Dispatchers.Main) {
            ObstacleViewModel.subscribeObstacleOption { option ->
